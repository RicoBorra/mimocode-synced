  ...
  it('handles bearer authorization and oauth client secrets', () => {
    ...
    expect(sanitizedConfig).toEqual({
      mcp: {
        github: {
          headers: {
            Authorization: 'Bearer {env:mimocode_mcp_GITHUB_AUTHORIZATION}',
          },
          oauth: {
            clientId: 'public',
            clientSecret: '{env:mimocode_mcp_GITHUB_OAUTH_CLIENT_SECRET}',
          },
        },
      },
    });
  ...
  it('preserves other authorization schemes', () => {
    ...
    expect(sanitizedConfig).toEqual({
      mcp: {
        gitlab: {
          headers: {
            Authorization: 'Token {env:mimocode_mcp_GITLAB_AUTHORIZATION}',
          },
        },
      },
    });
